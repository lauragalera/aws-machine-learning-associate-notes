...
Ensuring consistent preprocessing between training and inference phases is critical for reliable model predictions. The best practice is to reuse the exact normalization parameters (mean and standard deviation) computed during training rather than recalculating them on new data. Exporting the z-score normalization recipe from AWS Glue DataBrew and applying it to real-time data guarantees that the same parameters and transformation logic are used. This approach preserves the integrity of the data distribution expected by the model, thereby maintaining prediction accuracy and stability. Other options that recalculate parameters on real-time data introduce variability and inconsistency, which can degrade model performance.

Applying a log transformation is a common technique to reduce the impact of outliers by compressing the scale of numeric data, and this can be done in DataBrew without coding.
